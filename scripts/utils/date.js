...
export function parseDateValue(value, { maxDate = getLastAvailableDate() } = {}) {
  if (!value || !value.trim()) {
    return "";
  }
  ...
  if (parsedDate > latestAllowedDate) {
    throw new Error("A data selecionada deve ser anterior ou igual à data mais recente disponível.");
  }
...
export function formatDate(dateString) {
  ...
  const value = new Date(dateString.includes("T") ? dateString : `${dateString}T00:00:00Z`);
